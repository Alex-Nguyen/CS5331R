...
portion <-0.25 # percent of missing values to occupy the data. 0.02 = 2 %
...
m <-lm(missing_data$Sepal.Length ~ missing_data$Sepal.Width, data=missing_data)
missing_data
for(i in 1:nrow(missing_data))
{
  if(is.na(missing_data$Sepal.Length[i]))
  {
    missing_data$Sepal.Length[i] = coef(m)[1] + coef(m)[2]*missing_data$Sepal.Width[i]
  }
}
# 
